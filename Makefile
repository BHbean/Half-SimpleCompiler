...
grammar: flex.l grammar.y
ifeq ($(GRAMMAREXIST),notexist)
	mkdir $(GRAMMARFOLDER)
endif
	$(BISON) --output="./Linux/grammar.tab.cpp" --defines="./Linux/grammar.tab.h" ./grammar.y
	$(FLEX) --outfile="./Linux/lexer.flex.cpp" ./flex.l
	sed -i "1i\#include \"../tree/nodes.h\"" ./Linux/grammar.tab.h
	sed -i "1i\#include \"../symbol/symbol.h\"" ./Linux/grammar.tab.h
